...
| `native_libs_reload_e2e_test.dart` | macOS | A hot reload whose **own rebuild** moved a native library must withhold the increment. Over `e2e/codegen`, the one workspace that is source-assembled (so a reload runs `bazel build`) *and* bundles a `dlopen`ed library: edits `native_add/native/native_add.c` and a Dart string together, then asserts the reply names the library (`nativeLibsStale`), the window still shows the **old** Dart — which is what makes `runningCode: unchanged` checkable rather than self-reported — `app.restart` relaunches and shows the new Dart, and the reload after that succeeds. The last step is the guard against the opposite bug: a relaunch that failed to re-baseline would withhold every reload for the rest of the run. Waits for `frontend_server_ready` before editing, because an edit inside the assembler's own build window is compiled into the baseline (see `NativeLibsWatch.of`) |
...
| dev_tool native-library checks (`native_libs_watch.dart`, `native_libs_fingerprint.dart`, `native_libs_relauncher.dart`, the `nativeLibs` field of `_dev_config.json`) | `native_libs_reload_e2e_test.dart` (reload withheld) **and** `relaunch_e2e_test.dart` (restart relaunches) — the two halves of one contract, and no unit test can reach either |
