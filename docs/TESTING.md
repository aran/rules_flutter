...
- Android workspaces whose plugins declare `<uses-permission>` in their library manifests (e.g. `record_android`'s RECORD_AUDIO) need `common --merge_android_manifest_permissions` in `.bazelrc`: Bazel's manifest merger strips library permissions by default, while AGP always merges them.
